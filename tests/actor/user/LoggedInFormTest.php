...
class LoggedInFormTest extends TestCase
{
	...
	public function testUpdateProfileRequiredFields() {
		if (config('actor.users.change_email')) {
	...
	public function testUpdateProfileForm() {
		...
		if (config('actor.users.change_email')) {
			$this->actingAs($this->user)
				...
				->seeInDatabase(config('actor.users_table'), ['email' => '2_'.$this->user->email, 'name'  => $this->user->name.'_'.$rand]);
		} else {
			$this->actingAs($this->user)
				->visit('/account')
				->see('My Account')
				...
				->seePageIs('/account')
				->see('Profile successfully updated.')
				->seeInDatabase(config('actor.users_table'), ['name'  => $this->user->name.'_'.$rand]);
		}
	}
