...
class RoleEventListener
{
	...
	public function onCreated($event) {
		history()->log(
			$this->history_slug,
			'trans("history.backend.roles.created") '.$event->role->title,
			$event->role->id,
			'plus',
			'bg-green'
		);
	...
	public function onUpdated($event) {
		history()->log(
			$this->history_slug,
			'trans("history.backend.roles.updated") '.$event->role->title,
			$event->role->id,
			'save',
			'bg-aqua'
		);
	...
	public function onDeleted($event) {
		history()->log(
			$this->history_slug,
			'trans("history.backend.roles.deleted") '.$event->role->title,
			$event->role->id,
			'trash',
			'bg-maroon'
		);
